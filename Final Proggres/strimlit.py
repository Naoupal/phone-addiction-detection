...
@st.cache_resource
def load_models():
    """Load all models and artifacts from Hugging Face Hub"""
    REPO_ID = "Naoupal/phone-addiction-detection"
    SUBFOLDER = "save_model"
    ...
    xgb_model  = joblib.load(hf_download("xgb_cnn_augmented.pkl"))
    meta_model = joblib.load(hf_download("meta_model.pkl"))
    scaler     = joblib.load(hf_download("scaler.pkl"))
    col_order  = joblib.load(hf_download("col_order.pkl"))

    cnn_path   = hf_download("cnn_extractor.keras")
    cnn_model  = load_model(cnn_path)

    meta_path  = hf_download("metadata.json")
...
def predict_addiction(input_data, xgb_model, cnn_model, meta_model,
    ...
    raw_prob = meta_model.predict_proba(stack)[0]

    target_names = ["Mild", "Moderate", "Severe"]

    # Map probabilities to all 3 classes (meta_model may have been
    # trained on fewer classes than 3)
    full_prob = np.zeros(3)
    for i, cls in enumerate(meta_model.classes_):
        full_prob[int(cls)] = raw_prob[i]

    label      = target_names[pred]
    confidence = float(full_prob[pred])

    return label, confidence, {
        "Mild": float(full_prob[0]),
        "Moderate": float(full_prob[1]),
        "Severe": float(full_prob[2])
    }, {
...
def main():
    ...
    try:
        ...
        models_loaded = False
        st.error(f"⚠️ Gagal memuat model dari Hugging Face. Pastikan koneksi internet tersedia. Error: {e}")
